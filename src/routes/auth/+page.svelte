  ...
  const otpForm = createForm(() => ({
    ...
    onSubmit: async ({ value }) => {
      const { data } = await authClient.signIn.emailOtp({
        ...
        fetchOptions: {
          headers: {
            "x-captcha-response": verificationForm.state.values.cloudflareTurnstileToken,
          },
        },
      });
...
<main class="flex min-h-screen items-center justify-center p-4">
  <div class="grid w-full max-w-sm place-items-center">
    {#key step}
      <div
        class="col-start-1 row-start-1 w-full"
        in:fly={{ duration: 300, easing: sineOut, x: 16 }}
        out:fly={{ duration: 300, easing: sineOut, x: -16 }}
      >
        <Card.Root>
          {#if step === "email"}
            <Card.Header>
